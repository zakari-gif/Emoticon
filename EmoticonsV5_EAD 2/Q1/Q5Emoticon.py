...
class Emoticon:
    ...
    def headToArea(self, position):
        x=position[0]+self.sensor.generalConfiguration.get_screen().get_width()//2
        y=self.sensor.generalConfiguration.get_screen().get_height()//2-position[1]
        return [x,y]
    def color(self, x):
        couleur=[]
        if x==0:
    ...
    def head(self, x):
        ...
        pygame.draw.circle(self.sensor.generalConfiguration.screen, couleur,self.headToArea([0,0]),self.sensor.generalConfiguration.emoticonSize/2)   
    ...
    def mouth(self,position,x):
        if x>0.15:
            pygame.draw.arc(self.sensor.generalConfiguration.screen,[0,0,0],[position[0]-self.mouthMaxWidth/2,position[1]-self.mouthMaxHeight, self.mouthMaxWidth, self.mouthMaxHeight], math.pi+self.mouthAngle ,2*math.pi-self.mouthAngle)
        
        elif x<-0.15:
            pygame.draw.arc(self.sensor.generalConfiguration.screen,[0,0,0],[position[0]-self.mouthMaxWidth/2,position[1]-self.mouthMaxHeight/2, self.mouthMaxWidth, self.mouthMaxHeight],self.mouthAngle ,math.pi-self.mouthAngle)
        elif -0.15<x<0.15:
            pygame.draw.arc(self.sensor.generalConfiguration.screen,[0,0,0],[position[0]-self.mouthMaxWidth/2,position[1]],[position[0]+self.mouthMaxWidth/2,position[1]])
